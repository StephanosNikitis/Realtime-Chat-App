...
async function getAIResponse(chatId: string, userId: string) {
  const convoAI = await UserModel.findOne({ isAI: true });
  if(!convoAI) throw new NotFoundException("AI User not found");
  ...
  const result = await streamText({
    ...
    system: "You are Convo AI, a helpful and friendly assistant. Respond only with text and attend to the last user message only.",
  })
