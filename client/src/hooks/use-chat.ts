...
export const useChat = create<ChatState>()((set, get) => ({
  ...
  addNewMessage: (chatId, message) => {
    ...
    //   });
    // }
    get().addOrUpdateMessage(chatId, message);
  },
  ...
  addOrUpdateMessage: (chatId: string, msg: MessageType, tempId?: string) => {
    ...
    const tempMsgIndex = tempId 
    ? messages.findIndex((m) => m._id === tempId) 
    : -1;

    const finalMsgIndex = messages.findIndex((m) => m._id === msg._id);

    let updatedMessages;
    if (tempMsgIndex !== -1) {
      updatedMessages = messages.map((message, index) => 
      index === tempMsgIndex ? {...msg} : message);

      if(finalMsgIndex !== -1 && finalMsgIndex !== tempMsgIndex) {
        updatedMessages = updatedMessages.filter((_, i) => i !== finalMsgIndex);
      }
    } else if (finalMsgIndex !== -1) {
      updatedMessages = messages.map((message, index) =>
      index === finalMsgIndex ? {...message, ...msg} : message);
    } else {
